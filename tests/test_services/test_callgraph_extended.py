...
class TestCallGraphServiceExtended:
    ...
    async def test_get_call_graph_with_options(self, mock_query_executor):
        ...
        # 测试不同选项组合
        result1 = await service.get_call_graph("test", include_callers=True, include_callees=False)
        assert result1["success"] is True
        
        result2 = await service.get_call_graph("test", include_callers=False, include_callees=True)
        assert result2["success"] is True
        
        result3 = await service.get_call_graph("test", include_callers=False, include_callees=False)
    ...
    async def test_get_callees_multi_depth(self, mock_query_executor):
        ...
        callees_data = [
            {"name": f"func_{i}", "filename": "test.c"}
            for i in range(10)
        ]
        ...
        mock_query_executor.execute = AsyncMock(
            return_value={
                "success": True,
                "stdout": json.dumps(callees_data)
            }
        )
